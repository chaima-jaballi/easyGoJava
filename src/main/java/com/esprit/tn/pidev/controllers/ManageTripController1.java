...
public class ManageTripController1 {
    ...
    private void handleModifyReservation() {
        ...
        contributionField.setText(String.valueOf(tripToUpdate.getContribution()));
        ...
        tripTypeCombo.valueProperty().addListener((obs, oldVal, newVal) -> {
            if (newVal != null) {
                ...
                updateContribution(seatsField, tripTypeCombo, contributionField);
            }
        });

        // Gestion du changement du nombre de places
        seatsField.textProperty().addListener((obs, oldVal, newVal) -> {
            if (!newVal.matches("\\d*")) {
                seatsField.setText(newVal.replaceAll("[^\\d]", ""));
            }
            updateContribution(seatsField, tripTypeCombo, contributionField);
        });
        ...
        dialog.setResultConverter(buttonType -> {
            if (buttonType == saveButtonType) {
                if (!validateInputs(departureField.getText(), destinationField.getText(),
                        ...
                        returnTimeCombo.getValue(), seatsField.getText(), tripTypeCombo.getValue())) {
                    return null;
                }
    ...
    private void updateContribution(TextField seatsField, ComboBox<String> tripTypeCombo, TextField contributionField) {
        try {
            if (!seatsField.getText().isEmpty() && tripTypeCombo.getValue() != null) {
                int seats = Integer.parseInt(seatsField.getText());
                double contribution = calculateContribution(seats, tripTypeCombo.getValue());
                contributionField.setText(String.valueOf(contribution));
            }
    ...
    private double calculateContribution(int seats, String tripType) {
        switch (tripType) {
            case "Aller-retour":
                return 10 * seats;
            case "Aller seulement":
    ...
    private boolean validateInputs(String departure, String destination, LocalDate date,
                                   String depTime, String retTime, String seats, String tripType) {
